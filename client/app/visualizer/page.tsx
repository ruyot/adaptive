...
import {
    fetchRepoData as fetchRepoDataAPI,
...
function processRepoData(data: any[]) {
    ...
    Object.keys(filesByType).forEach((type) => {
        if (filesByType[type].length > 0) {
            const {
                tree,
                explanations: typeExplanations,
                fileContents: typeContents,
            } = createFileTree(filesByType[type], type);
...
function CodebaseVisualizerContent() {
    ...
    const loadRepository = async () => {
        ...
        if (!parsedRepo) {
            setError(
                "Invalid GitHub URL. Please use format: github.com/owner/repo"
            );
            return;
        }
    ...
    // NOTE: This is called twice in dev due to how react works.
    useEffect(() => {
        console.log("repo");
        if (repoUrl) {
            loadRepository();
    ...
    const handleAnalyzeFile = async (path: string, content: string) => {
        try {
            ...
            const actualContent = selectedFile
    ...
    const currentFiles = selectedSection
        ? sections[selectedSection]?.files || []
        : [];
                ...
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-8">
                    <div className="pointer-events-auto floating-card">
                        <ExplanationPanel
                            ...
                            filePath={currentExplanation.details[0]?.replace(
                                "Path: ",
                                ""
                            )}
                            analysisResult={
